...
@WebMvcTest(BabyControllerAdmin.class)
@WithMockUser(username = "testUser", roles = { "USER" })
@ActiveProfiles("test")
public class BabyControllerAdminTest {
    ...
    public void TestFindBabies() throws Exception {
        ...
        Mockito.when(babyService.getAll()).thenReturn(babies);
        Mockito.when(authoritiesService.isAdmin()).thenReturn(true);

        mockMvc.perform(get("/api/v1/admin/baby").header("Authorization", "Bearer " + token)).andExpect(status().isOk())
    ...
    public void TestFindById() throws Exception {
        Baby b1 = createDummyBaby(1);
        Mockito.when(babyService.findByIdAdmin(1)).thenReturn(b1);
        Mockito.when(authoritiesService.isAdmin()).thenReturn(true);

        mockMvc.perform(get("/api/v1/admin/baby/1").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk()).andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.foodPreference", is("Test food preference")));
    }
    ...
    public void TestCreateBaby() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/admin/baby").header("Authorization", "Bearer " + token).with(csrf())
    ...
    public void TestUpdateBaby() throws Exception {
        ...
        Mockito.when(babyService.updateBabyAdmin(Mockito.eq(1), Mockito.any(BabyDTO.class))).thenReturn(returnedBaby);
        Mockito.when(authoritiesService.isAdmin()).thenReturn(true);

        mockMvc.perform(put("/api/v1/admin/baby/1").header("Authorization", "Bearer " + token).with(csrf())
    ...
    public void testDeleteBaby() throws Exception {
        Mockito.doNothing().when(babyService).deleteBabyAdmin(1);
        Mockito.when(authoritiesService.isAdmin()).thenReturn(true);

        mockMvc.perform(delete("/api/v1/admin/baby/1").header("Authorization", "Bearer " + token).with(csrf()))
    ...
    public void TestGetBabyById_NotFound() throws Exception {
        Mockito.when(babyService.findByIdAdmin(999)).thenThrow(new ResourceNotFoundException("Not Found"));
        Mockito.when(authoritiesService.isAdmin()).thenReturn(true);

        mockMvc.perform(get("/api/v1/admin/baby/999").header("Authorization", "Bearer " + token))
                .andExpect(status().isNotFound());
    }

    @Test
    public void TestDeleteBaby_NotFound() throws Exception {
        Mockito.doThrow(new ResourceNotFoundException("Not Found")).when(babyService).deleteBabyAdmin(999);
        Mockito.when(authoritiesService.isAdmin()).thenReturn(true);

        mockMvc.perform(delete("/api/v1/admin/baby/999").header("Authorization", "Bearer " + token).with(csrf()))
                .andExpect(status().isNotFound());
    }
