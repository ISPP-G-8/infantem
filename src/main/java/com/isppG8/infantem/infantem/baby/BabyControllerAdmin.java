...
@Tag(name = "BabyAdmin", description = "Gestión de bebés admin")
@RestController
@RequestMapping("/api/v1/admin/baby")
public class BabyControllerAdmin {
    ...
    @Autowired
    public BabyControllerAdmin(BabyService babyService, AuthoritiesService authoritiesService) {
    ...
    public ResponseEntity<List<BabyDTO>> getAllBabies() {
        ...
        if (authoritiesService.isAdmin()) {
            List<BabyDTO> babies = babyService.getAll().stream().map(baby -> new BabyDTO(baby)).toList();
    ...
    public ResponseEntity<BabyDTO> findById(@PathVariable Integer id) {
        if (authoritiesService.isAdmin()) {
            Baby baby = babyService.findByIdAdmin(id);
    ...
    public ResponseEntity<BabyDTO> updateBaby(@PathVariable("id") Integer id, @RequestBody @Valid BabyDTO baby) {
        if (authoritiesService.isAdmin()) {
            Baby updatedBaby = babyService.updateBabyAdmin(id, baby);
    ...
    public ResponseEntity<Void> deleteBaby(@PathVariable("id") Integer id) {
        if (authoritiesService.isAdmin()) {
            this.babyService.deleteBabyAdmin(id);
            return ResponseEntity.noContent().build();
        }
        return null;
    }
