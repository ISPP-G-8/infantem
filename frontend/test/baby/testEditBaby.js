...
(async function testLogInLogOut() {
  ...
  try {
    ...
    let añadirbebesButton = await driver.findElement(By.xpath("//div[contains(text(), 'Editar')]"));
    await añadirbebesButton.click();

    await driver.sleep(3000);

    let nombreBebeInput = await driver.findElement(By.css("input[placeholder='Nombre']"));
    await nombreBebeInput.clear();
    ...
    await fechanacimientoInput.clear();
    ...
    await preferenciasInput.clear();
    await preferenciasInput.sendKeys("Potitos");

    await driver.sleep(1000);

    let guardarbebesButton = await driver.findElement(By.xpath("//div[contains(text(), 'Actualizar')]"));
